...
@require_POST
@login_required
def system_control_add(request, system_id):
    """Add selected Control to System's selection  &  redirect to System Controls page"""
    return system_controls_modify(request, system_id, 'add_control', 'Added', 'to',
...
@require_POST
@login_required
def system_control_remove(request, system_id, system_control_id):
    """Remove selected Control from System's selection,  deleting associated System Statements,  &  redirect to System Controls page"""
    return system_controls_modify(request, system_id, 'remove_control', 'Removed', 'from',
        system_control_id,
        lambda system, control_key:  system.remove_control(system_control_id)
    )

def system_controls_modify(request, system_id, action, action_user_verb_after, action_user_to_or_from,
    ...
    def log_system_controls_change(action, control_or_id, user_verb_after_or_message,
        ...
        if isinstance(control_or_id, ElementControl):
            user_verb_after_or_message = f"{user_verb_after_or_message} {control_or_id.get_name_with_catalog()} {action_user_to_or_from} this System's selected Controls"
...
def system_controls_export_xacta_xslx(request, system_id):
    """Export System's selected Controls in an Xacta 360 compatible MS-XSLX Microsoft Excel .xslx format"""
